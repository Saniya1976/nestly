...
function CreatePost() {
  ...
  const handleGenerateFromText = async () => {
    ...
    setIsGenerating(true);
    try {
      console.log("📱 Mobile Debug: Starting generation with prompt:", aiPrompt);
      console.log("📱 Mobile Debug: Environment:", {
      ...
      const result = await generateCaptionFromText(aiPrompt);
      console.log("📱 Mobile Debug: Server response:", result);
      ...
      if (result.success && result.caption) {
        ...
        console.error("📱 Mobile Debug: Error from server:", result.error);
        toast.error(result.error || "Failed to generate caption");
      }
    } catch (error: any) {
      console.error("📱 Mobile Debug: Catch error:", error);
  ...
  const handleImproveCaption = async () => {
    ...
    setIsGenerating(true);
    try {
      console.log("📱 Mobile Debug: Improving caption:", content.substring(0, 50));
      const result = await improveCaption(content);
      console.log("📱 Mobile Debug: Improve response:", result);
      ...
      console.error("📱 Mobile Debug: Improve error:", error);
      toast.error("Network error. Check connection and try again.");
    } finally {
      setIsGenerating(false);
    }
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex items-center justify-between sm:justify-start gap-2 w-full sm:w-auto">
              ...
              >
                ...
                <span className="xs:hidden sm:inline">Photo</span>
              </Button>
              ...
              {!showAIPrompt && (
                ...
                >
                  ...
                  <span className="xs:hidden sm:inline">Generate</span>
                </Button>
              )}
              ...
              {content.trim() && !showAIPrompt && (
                ...
                >
                  ...
                  <span className="xs:hidden sm:inline">Improve</span>
                </Button>
              )}
            </div>
